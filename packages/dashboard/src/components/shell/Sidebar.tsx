...
export function Sidebar({ collapsed, onNavigate }: SidebarProps) {
  ...
  // NavLink's function-form className cannot be used here: Radix's `asChild`
  // (used for the collapsed tooltips) merges className as a string, which
  // stringifies the function and applies no classes at all. So active state is
  // computed here and passed as a plain string.
    ...
    <div className="flex h-full flex-col border-r border-border bg-card">
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3">
        {groups.map((group, groupIndex) => (
          <div key={group.label} className={cn(collapsed ? 'mb-2' : 'mb-4')}>
            {collapsed && groupIndex > 0 && (
              <div className="mx-auto mb-2 w-8 border-t border-border" aria-hidden="true" />
            )}
            {!collapsed && (
                ...
                const link = (
                  <NavLink
                    ...
                    className={cn(
                      ...
                      collapsed ? 'mx-auto h-9 w-9 justify-center' : 'gap-3 px-2 py-2',
